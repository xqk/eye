...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        version, is_repair = settings.eye_VERSION, False
        res = requests.get(f'https://api.eye.icl.site/apis/release/latest/?version={version}').json()
        if res['error']:
        ...
        # update web
        web_dir = os.path.join(settings.BASE_DIR, '../eye_web')
        commands = [
            f'curl -o /tmp/eye_web.tar.gz https://cdn.eye.icl.site/eye/web_{version}.tar.gz',
            f'rm -rf {web_dir}/build',
            f'tar xf /tmp/eye_web.tar.gz -C {web_dir}'
        ]
        ...
        commands = [
            ...
            f'python3 ./tools/migrate.py {settings.eye_VERSION}'
        ]
        task = subprocess.Popen(' && '.join(commands), shell=True)
        if task.wait() != 0:
