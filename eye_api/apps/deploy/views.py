...
def post_request_ext1(request):
    ...
    if error is None:
        ...
        form.eye_version = Repository.make_eye_version(deploy.id)
        if form.extra[0] == 'tag':
            if not form.extra[1]:
            ...
            form.eye_version = repository.eye_version
...
def post_request_ext1_rollback(request):
    ...
    if error is None:
        ...
        versions = list({x.eye_version: 1 for x in requests}.keys())
        if req.eye_version not in versions[:req.deploy.extend_obj.versions + 1]:
