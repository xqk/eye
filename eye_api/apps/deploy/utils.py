...
from apps.deploy.helper import Helper, eyeError
...
def dispatch(req):
    ...
    try:
        ...
        env = AttrDict(
            eye_APP_NAME=req.deploy.app.name,
            eye_APP_KEY=req.deploy.app.key,
            eye_APP_ID=str(req.deploy.app_id),
            eye_REQUEST_ID=str(req.id),
            eye_REQUEST_NAME=req.name,
            eye_DEPLOY_ID=str(req.deploy.id),
            eye_ENV_ID=str(req.deploy.env_id),
            eye_ENV_KEY=req.deploy.env.key,
            eye_VERSION=req.version,
            eye_BUILD_VERSION=req.eye_version,
            eye_DEPLOY_TYPE=req.type,
...
def _ext1_deploy(req, helper, env):
    if not req.repository_id:
        rep = Repository(
            ...
            version=req.version,
            eye_version=req.eye_version,
            extra=req.extra,
            remarks='eye AUTO MAKE',
        ...
        with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for t in futures.as_completed(threads):
                ...
                if exception:
                    ...
                    if not isinstance(exception, eyeError):
...
def _ext2_deploy(req, helper, env):
    ...
    env.update({'eye_RELEASE': req.version})
    if req.version:
        for index, value in enumerate(req.version.split()):
            env.update({f'eye_RELEASE_{index + 1}': value})
    ...
    for action in host_actions:
        if action.get('type') == 'transfer':
            ...
            tar_gz_file = f'{req.eye_version}.tar.gz'
    ...
    if host_actions:
        if req.deploy.is_parallel:
            ...
            with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                for h_id in json.loads(req.host_ids):
                    new_env = AttrDict(env.items())
                    t = executor.submit(_deploy_ext2_host, helper, h_id, host_actions, new_env, req.eye_version)
                    t.h_id = h_id
                    threads.append(t)
                for t in futures.as_completed(threads):
                    exception = t.exception()
            ...
            while host_ids:
                ...
                new_env = AttrDict(env.items())
                try:
                    _deploy_ext2_host(helper, h_id, host_actions, new_env, req.eye_version)
                except Exception as e:
                    helper.send_error(h_id, f'Exception: {e}', False)
                    for h_id in host_ids:
                        helper.send_error(h_id, '终止发布', False)
...
def _deploy_ext1_host(req, helper, h_id, env):
    ...
    with host.get_ssh(default_env=env) as ssh:
        ...
        if code == 0:
            helper.send_error(host.id, f'检测到该主机的发布目录 {extend.dst_dir!r} 已存在，为了数据安全请自行备份后删除该目录，eye 将会创建并接管该目录。')
        if req.type == '2':
            ...
            command = f'cd {extend.dst_repo} && rm -rf {req.eye_version} && tar xf {tar_gz_file} && rm -f {req.deploy_id}_*.tar.gz'
            helper.remote_raw(host.id, ssh, command)
            helper.send_step(h_id, 1, '\033[32m完成√\033[0m\r\n')

        # pre host
        repo_dir = os.path.join(extend.dst_repo, req.eye_version)
...
def _deploy_ext2_host(helper, h_id, actions, env, eye_version):
            ...
            if action.get('type') == 'transfer':
                if action.get('src_mode') == '1':
                    try:
                        ...
                        ssh.put_file(os.path.join(REPOS_DIR, env.eye_DEPLOY_ID, eye_version), dst)
                    except Exception as e:
                        helper.send_error(host.id, f'Exception: {e}')
                    ...
                    sp_dir, sd_dst = os.path.split(action['src'])
                    tar_gz_file = f'{eye_version}.tar.gz'
                    try:
                        ssh.put_file(os.path.join(sp_dir, tar_gz_file), f'/tmp/{tar_gz_file}')
                    except Exception as e:
                        helper.send_error(host.id, f'Exception: {e}')

                    command = f'mkdir -p /tmp/{eye_version} && tar xf /tmp/{tar_gz_file} -C /tmp/{eye_version}/ '
                    command += f'&& rm -rf {action["dst"]} && mv /tmp/{eye_version}/{sd_dst} {action["dst"]} '
                    command += f'&& rm -rf /tmp/{eye_version}* && echo "transfer completed"'
            else:
