...
class SSH:
    ...
    def _get_channel(self):
        ...
        while True:
            if self.channel.recv_ready():
                ...
                self.client.close()
                raise Exception('Wait eye response timeout')
            else:
                counter += 1
                time.sleep(0.1)
        return self.channel
    ...
    def _handle_command(self, command, environment):
        ...
        if not self.already_init:
            commands = 'export eye_EXEC_FILE=$(mktemp)\n'
            commands += 'trap \'rm -f $eye_EXEC_FILE\' EXIT\n'
        ...
        commands += f'echo {b64_command.decode()} | base64 -di > $eye_EXEC_FILE\n'
        commands += 'source $eye_EXEC_FILE\n'
        return commands
