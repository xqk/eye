...
eye_server = 'https://api.eye.icl.site'
notify_source = 'monitor'


def send_login_wx_code(wx_token, code):
    url = f'{eye_server}/apis/login/wx/'
    eye_key = AppSetting.get_default('eye_key')
    res = requests.post(url, json={'token': eye_key, 'user': wx_token, 'code': code}, timeout=30)
...
class Notification:
    def __init__(self, grp, event, target, title, message, duration):
        ...
        self.eye_key = AppSetting.get_default('eye_key')
        self.u_ids = []
    ...
    def monitor_by_wx(self, users):
        ...
        self.handle_request(f'{eye_server}/apis/notify/wx/', data, 'eye')
    ...
    def monitor_by_email(self, users):
        ...
        if mail_service.get('server'):
            ...
            self.handle_request(f'{eye_server}/apis/notify/mail/', data, 'eye')
        else:
