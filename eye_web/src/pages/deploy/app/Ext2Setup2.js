      ...
      <Form labelCol={{span: 6}} wrapperCol={{span: 14}} className={styles.ext2Form}>
        {store.deploy.id === undefined && (
          <Alert
            ...
            description={[
              <p key={1}>eye 将遵循先本地后目标主机的原则，按照顺序依次执行添加的动作，例如：本地动作1 -> 本地动作2 -> 目标主机动作1 -> 目标主机动作2 ...</p>,
              <p key={2}>执行的命令内可以使用发布申请中设置的环境变量 eye_RELEASE，一般可用于标记一次发布的版本号或提交ID等，在执行的脚本内通过使用 $eye_RELEASE
                获取其值来执行相应操作。</p>,
              <p key={3}>{Tips}。</p>
            ]}/>
        )}
            ...
            {item['type'] === 'transfer' ? ([
              <Form.Item key={0} required label="数据来源">
                <Input
                  ...
                  placeholder="请输入本地（部署eye的容器或主机）路径"
              ...
              <Form.Item key={2} required label="目标路径" extra={<a
                ...
                href="https://eye.icl.site/docs/deploy-config#transfer">使用前请务必阅读官方文档。</a>}>
                <Input
